...
const kundeAnmeldelser = [
  {
    navn: "Jens",
    citat:
      "Jeg havde aldrig trænet før, men med Mohammad's hjælp følte jeg mig tryg fra dag et.",
    stjerner: 4,
  ...
  },
  {
    navn: "Susanne",
    citat:
      "Mohammad's personlige tilgang har fuldstændig forandret min måde at tænke træning på.",
    stjerner: 5,
  ...
  },
  {
    navn: "Lars",
    citat:
      "Utroligt professionel træner der virkelig forstår at motivere og guide.",
    stjerner: 4,
    billede: "images/transformationsbilledeto.png",
  },
]; //Indeholder 3 objekter, er nemt og overskuelig måde at sætte op, nemt at tilføje nye værdier til som feks dato, du kan nemt tilføje nye anmeldelser ved at bruge push værdier til at sætte dem ind i arrayet.
...
function opdaterAnmeldelse() {
  ...
  const desktopCitat = document.querySelector(
    ".sektion-anmeldelser .anmeldelser-citat"
  );
  const desktopNavn = document.querySelector(
    ".sektion-anmeldelser .anmeldelser-tekst p:last-child"
  );
  const desktopBillede = document.querySelector(
    ".sektion-anmeldelser .anmeldelser-billede"
  );
  const desktopStjerner = document.querySelector(
    ".sektion-anmeldelser .anmeldelser-stjerner"
  );

  // Vælg elementer til mobil-visning
  const mobilCitat = document.querySelector(
    ".sektion-anmeldelser-mobil .anmeldelser-citat-mobil"
  );
  const mobilNavn = document.querySelector(
    ".sektion-anmeldelser-mobil .anmeldelser-tekst-mobil p:last-child"
  );
  const mobilBillede = document.querySelector(
    ".sektion-anmeldelser-mobil .anmeldelser-billede-mobil"
  );
  const mobilStjerner = document.querySelector(
    ".sektion-anmeldelser-mobil .anmeldelser-stjerner"
  );

  // Opdater citater, tilføjer gåseøjne til teksten
  if (desktopCitat)
    desktopCitat.textContent = '"' + aktuelAnmeldelse.citat + '"';
  ...
  function opretStjerneIkoner(stjerner) {
    let stjerneHtml = ""; //Tom tekststreng til at vise stjernerne i hTML
    // Opret 5 stjerner, hvor nogle er udfyldt baseret på anmeldelsesstjerners antal (Bliver oprettet en for-løkke fordi der er 5 stjerner, så processen skal gentages 5 gange)
    for (let i = 0; i < 5; i++) {
      // Let i = 0 - er starten, her bliver variablen I sat til at være 0, det sker kun 1 gang. Efter dette kommer (i<5), dette er løkken som fortsætter så længe i er mindre end 5. i++ er her vi tæller hvor langt vi er. Den vil hver gang i løkken blive forhøjet med 1 så den til sidst ender med at sige 5<5 og derfor stopper løkken. Det er det samme som at skive i= i + 1.
      stjerneHtml +=
        '<img src="ikoner/' +
        (i < stjerner ? "udfyldtstjerne" : "tomstjerne") + //Kaldes en ternary operator, en kort if/else sætning, hvis i < stjerne er sandt vælger udfyldt stjerne, ellers vælger den tom stjerne
        '.svg" alt="Stjerne">'; // til start vil den kunne opdatere hjemmesiden HTML fil, og til føje de stjerner som skal ind.
  ...
  //For-løkken går 5 gange (en gang for hver stjerne). Den vælger udfyldt eller tom stjerne baseret på antallet af stjerner (Med ternary operator en forkoftet if/else sætning). Resultatet bliver en række stjerneikoner, der vises på hjemmesiden.

  // Opdater stjerner
  if (desktopStjerner)
    desktopStjerner.innerHTML = opretStjerneIkoner(aktuelAnmeldelse.stjerner);
  if (mobilStjerner)
    mobilStjerner.innerHTML = opretStjerneIkoner(aktuelAnmeldelse.stjerner);
}
...
function næsteAnmeldelse() {
  ...
  anmeldelsesReviewIndex =
    (anmeldelsesReviewIndex + 1) % kundeAnmeldelser.length;
  opdaterAnmeldelse();
}
